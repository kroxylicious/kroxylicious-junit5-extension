...
while getopts ":l:v:b:k:r:n:w:s:h" opt; do
  case $opt in
    ...
    h)
      1>&2 cat << EOF
usage: $0 -k keyid -v version -l relcand-label [-b branch] [-r repository] [-s] [-h] [-w workbranch] [-n nextversion]
...
mvn -Prelease -DskipTests=true -DreleaseSigningKey="${GPG_KEY}" -DprocessAllModules=true deploy | tee ${MVN_DEPLOY_OUTPUT}
...
gh pr create --head "${PREPARE_DEVELOPMENT_BRANCH}" \
             --base "${BRANCH_FROM}" \
             --title "Kroxylicious junit extension development version ${RELEASE_VERSION}" \
             --body "${BODY}" \
